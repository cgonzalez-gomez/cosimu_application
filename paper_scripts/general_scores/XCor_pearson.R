...
if(!paired_est){
  if(is.na(topN)){
    if(is.null(mat_pval)){
      set.seed(100)
      tic("xpear")
      res_xpear<- purrr::map(seq(1,ncol(mat_queries)),function(i){
        fc <- mat_queries[,i]
        names(fc) <- gene_names
        S <-get_deg(fc, abs_thresh = 1.5, pval = NA, use.pval = FALSE, thresh_pval = 0.05)
        xpear <- XCor_score(S =S, gene_names = gene_names,
                            pert_names = pert_names,list_ranked_R = L3,
                            ncpus = ncpus,permuted_pval = permuted_pval,permute_nb = 100,method = "pearson")
...
saveRDS(res_xpear,paste0(save_path,"_",topN,".rds"))
